...
async def smart_tool_prefetch(state: State, config: RunnableConfig) -> Dict[str, Any]:
    ...
    tasks.append(asyncio.create_task(_safe_rag_search(rag_query, thread_id=thread_id)))
    ...
    try:
        ...
        if result.get("status") == "no_results":
            ...
            if is_netz_query:
                ...
                logger.info(f"🔧 Carbon AIX 질문 감지 → MCP 도구 사용 대기")
                return result
...
async def call_model(
    ...
    model = llm.bind_tools(all_tools) # claude에게 도구 목록 전달
