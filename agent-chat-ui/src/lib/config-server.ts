...
/**
 * Loads chat categories from YAML file on the server side.
 *
 * @returns Categories data if successful, undefined otherwise
 */
async function loadServerChatCategories(): Promise<any | undefined> {
  try {
...
export async function loadServerConfig(): Promise<ChatConfig> {
  ...
  const categories = await loadServerChatCategories();
  if (categories) {
    (config as any).categories = categories;
  }
